...
*sigma.pathfinding.astar.js* is a plugin for [sigma.js](http://sigmajs.org) that computes path in a view
...
It adds a method to your `sigma.view` called `astar(srcId, destId[, options])`.
...
- `options` (optional), an object containing one or more of those properties:
   - `undirected` (default: `false`), if set to `true`, consider the view as non-oriented (will explore all edges, including the inbound ones);
