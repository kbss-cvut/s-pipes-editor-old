...
class GraphDaoTest extends BaseDaoTestRunner {
  ...
  def getAndAddGraph() {
    ...
    val e = new Edge(new URI("https://edges/11"), n, null)
    val nodes = Set[Node](n, n, n).asJava
    val edges = Set[Edge](e, e).asJava
  ...
  def getAllGraphs() {
    ...
    types.add("https://type/2")
    types.add("https://type/3")
    val n = new Node(new URI("https://nodes/11" + nodeCount), "Label", 1, 2, types, Set[String]().asJava, Set[String]().asJava)
    val e = new Edge(new URI("https://edges/11"), n, n)
    val nodes = Set[Node]().asJava
    val edges = Set[Edge]().asJava
